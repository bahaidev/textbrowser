...
- ***textbrowser.appcache*** - Offline AppCache manifest. You may base this off of `textbrowser-sample.appcache`, being sure to add your own data files for offline caching. If you are using `user.css` or `user.js`, be sure to uncomment in the copied file (HTML currently only allows one cache file apparently as per <https://html.spec.whatwg.org/multipage/semantics.html#attr-html-manifest>, so we apparently cannot add to this dynamically nor use data: URLs.)
...
1. Change paths. The path prefix `bower_components/textbrowser` in `textbrowser.appcache` and the `localeFileBasePath` property in `appdata/languages.json` will probably need to be stripped in this environment (or, for the latter case, a new file can be designated via the `languages` property in the `resources/user.js` call to the *TextBrowser* constructor).
