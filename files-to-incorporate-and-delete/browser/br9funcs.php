...
function make_table ($result) {
	...
	global $tr2open, $tr2close, $tdopen, $tdclose, $tr2divider, $tddivider;
	global $escapehtml;
...
*/
		...
		$tablearray[$trns][] = "		<".$tr2open." class='row row".$trns."'>\n			";
...
//							global $$field; // dynamically declare the field variables
							...
							$tablearrayprep = "<".$tdopen;
							$tablearrayprep .= <<<HERE
 id='cellfield{$k}_{$trns}' class="$fieldkeyclass cell $fieldauto" xml:lang='$orig_lang_code_field' dir='
HERE;
//'
							if ($lang_direction != "") {
								$tablearrayprep .= $lang_direction; // This will be obtained from the latest get_language call (which was called above by get_field)
...
//print "ct: ".$interlinarrcount."<br />";
							...
							elseif (!$escapehtml) {
								$tablearray[$trns][] = $tablearrayprep.$row[$$fieldauto].$td_interlin.$tddivider."</".$tdclose.">";
							}
							else {
			...
			if ($k < $cycleflds) {
				...
				if ($$option == "yes" && (strpos($$field, "_", 1))) {
					...
					$tablearrayprep = '<'.$tdopen.' id="cell'.$id2.'" class="'.$fieldautoclass.' cell '.$field.'" xml:lang="'.$auto_xmllang.'" dir="'.$textdirecauto.'">';
...
//print "ct: ".$interlinarrcount."<br />";
				...
				elseif ($$option == "yes") { // If the user opted to see that field
					...
					$tablearrayprep =  "<".$tdopen;
					$tablearrayprep .= <<<HERE
 id='cellfield{$k}_{$trns}' class="$fieldkeyclass cell $field" xml:lang='$orig_lang_code_field' dir='
HERE;
//'
					if ($lang_direction != "") {
						$tablearrayprep .= $lang_direction; // This will be obtained from the latest get_language call (which was called above by get_field)
...
//print "ct: ".$interlinarrcount."<br />";
					...
					elseif (!$escapehtml) {
						$tablearray[$trns][] = $tablearrayprep.$row[$$field].$td_interlin.$tddivider."</".$tdclose.">";
					}
					else {
						$tablearray[$trns][] = $tablearrayprep.htmlspecialchars($row[$$field], ENT_QUOTES, "UTF-8").$td_interlin.$tddivider."</".$tdclose.">";
					}
				} //end elseif (if the user opted to see the given field)
			} // end if (if not the last cycle)
		} //end for to go through all the fields
	$tablearray[$trns][] = $tr2divider."</".$tr2close.">"."\n";
	...
	if ($_GET['trnsps']=="1") { // Horizontal
		for ($j = 1; $j < $counttablearray-1; $j++) {
			...
			print $tablearray[1][$counttablearray];
		} // end for
	} // end if
